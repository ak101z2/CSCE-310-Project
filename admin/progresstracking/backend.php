<!-- Backend php file for CRUD form handling and view page -->
<!-- For each operation (insert, edit, delete) check to see if corresponding dropdown isn't null -->
<!-- If so then access all relevant data methods through _POST -->
<!-- Utilize SQL query for insert, edit, delete to perform relevant operation -->
<!-- Utilize SQL queries to display data for view page (incorporates view) -->
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['view']) && $_POST['view'] != 'null') {
            ...
            if ($_POST['view'] == 'student') {
                $selectedStudentUIN = $_POST['student'];
                echo "<title>Student Report</title>";
                echo "<h1>Student Report for Student (UIN: $selectedStudentUIN)</h1>";

                $qProgramsForStudent = "SELECT * FROM StudentPrograms WHERE UIN = '$selectedStudentUIN';";
